...
public class IntArray
{
	...
	public class Track_Deform : MonoBehaviour
	{
		private static bool enableGizmo = true;

		[Tooltip("Needs to be tweaked manually for each track")] public float offset;
		...
		void Update()
		{
			...
			for (int i = 0; i < anchorArray.Length; i++)
			{
				float tempDist = anchorArray[i].localPosition.y - initialPosArray[i] + offset;
		...
		void OnDrawGizmos()
		{
			if (anchorArray != null && anchorArray.Length != 0 && offsetArray != null && offsetArray.Length != 0 && enableGizmo)
			{
